...
export const BranchesPage = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingBranch) {
        ...
        if (!businessId) { console.error("No business ID available to create branch"); return; }
  ...
  const handleDelete = async (branchId: string) => {
    ...
    try {
      ...
      await fetchBranches();
      if (selectedBranch?._id === branchId) { setSelectedBranch(null); setMobilePanel("list"); }
    } catch (error) {
      console.error("Failed to delete branch:", error);
    }
  };

  const handleCancel = () => { setShowForm(false); setEditingBranch(null); setForm(emptyBranch); setMobilePanel("list"); };
  const handleAddNew = () => { setShowForm(true); setEditingBranch(null); setForm(emptyBranch); setMobilePanel("detail"); };
  const handleSelectBranch = (branch: TBranch) => { setSelectedBranch(branch); setShowForm(false); setMobilePanel("detail"); };
    ...
    return (
      <Card>
        <div className="flex flex-col items-center justify-center py-12">
          ...
          <p className="text-gray-600 mt-4 font-medium">{t("branches.loading")}</p>
        </div>
      </Card>
    );
  }
  ...
  return (
    <div className="h-full flex flex-col gap-4 overflow-hidden">
      <div className="flex-shrink-0 gap-2 flex items-center justify-between">
        <div>
          <SectionTitle
            title={t("branches.businessBranches")}
            subtitle={t("branches.manageBranches")}
          />
        </div>
        <Button variant="default" onClick={handleAddNew} className="flex items-center gap-2 text-sm sm:text-base">
      ...
      {mobilePanel === "detail" && (
        <div className="flex-shrink-0 lg:hidden">
          <button
            onClick={() => { setMobilePanel("list"); setShowForm(false); setEditingBranch(null); }}
      ...
      <div className="flex-1 overflow-hidden lg:grid lg:grid-cols-[minmax(300px,1fr)_2fr] lg:gap-5">
        <div className={`h-full overflow-y-auto ${mobilePanel === "list" ? "block" : "hidden"} lg:block`}>
          <BranchList
            branches={branches}
        ...
        <div className={`h-full overflow-y-auto ${mobilePanel === "detail" ? "block" : "hidden"} lg:block`}>
          {showForm ? (
            <BranchForm form={form} setForm={setForm} editingBranch={editingBranch} saving={saving} onSave={handleSave} onCancel={handleCancel} />
          ) : (
            <BranchMap branches={branches} selectedBranch={selectedBranch} />
          )}
        </div>
      </div>
    </div>
