...
import {
  TrendingUp, TrendingDown, DollarSign, Calendar, Award, Download, Activity,
} from "lucide-react";
import {
...
interface Specialist { name: string; }
interface Booking {
  _id?: string;
  bookingDate: string;
  createdAt: string;
...
interface ServiceDataEntry { name: string; count: number; revenue: number; }
interface SpecialistDataEntry { name: string; count: number; revenue: number; }
interface StatusPieEntry { name: string; value: number; color: string; }
interface RadialDataEntry { name: string; value: number; fill: string; }
...
export const AnalyticsPage = () => {
  ...
  const getFilteredBookings = (): Booking[] => {
    ...
    return bookings.filter((booking) => new Date(booking.bookingDate).getTime() >= cutoffDate.getTime());
  };
  ...
  const getPreviousPeriodBookings = (): number => {
    ...
    return bookings.filter((booking) => {
      ...
      return date.getTime() >= previousStart.getTime() && date.getTime() < currentStart.getTime();
    }).length;
  };
  ...
  const getDailyData = (): DailyDataEntry[] => {
    ...
    for (let i = days - 1; i >= 0; i--) {
      ...
      dailyMap.set(dateStr, { date: dateStr, bookings: 0, revenue: 0, completed: 0, confirmed: 0, pending: 0, cancelled: 0 });
    }
    filteredBookings.forEach((booking) => {
      const dateStr = new Date(booking.createdAt).toLocaleDateString("en-US", { month: "short", day: "numeric" });
      const data = dailyMap.get(dateStr);
      if (data) {
        data.bookings++;
        const status = booking.status;
        if (status in data) (data[status as keyof DailyDataEntry] as number)++;
        if (["confirmed", "completed"].includes(status)) data.revenue += booking.price?.amount || 0;
      }
    });
  ...
  const getServiceData = (): ServiceDataEntry[] => {
    ...
    filteredBookings.forEach((booking) => {
      const name = booking.service?.name || t("analytics.unknown");
      const existing = serviceMap.get(name) || { name, count: 0, revenue: 0 };
      existing.count++;
      if (["confirmed", "completed"].includes(booking.status)) existing.revenue += booking.price?.amount || 0;
      serviceMap.set(name, existing);
    });
    return Array.from(serviceMap.values()).sort((a, b) => b.count - a.count).slice(0, 6);
  };

  const getSpecialistData = (): SpecialistDataEntry[] => {
    const specialistMap = new Map<string, SpecialistDataEntry>();
    filteredBookings.forEach((booking) => {
      const name = booking.specialist?.name || t("analytics.unknown");
      const existing = specialistMap.get(name) || { name, count: 0, revenue: 0 };
      existing.count++;
      if (["confirmed", "completed"].includes(booking.status)) existing.revenue += booking.price?.amount || 0;
      specialistMap.set(name, existing);
    });
    return Array.from(specialistMap.values()).sort((a, b) => b.count - a.count).slice(0, 5);
  };
  ...
  const radialData: RadialDataEntry[] = [
    { name: t("analytics.completionRate"), value: completionRate, fill: "#5D6B8D" },
  ];

  const statusPieData: StatusPieEntry[] = [
    { name: t("statuses.completed"), value: completedBookings, color: "#5D6B8D" },
    { name: t("statuses.confirmed"), value: confirmedBookings, color: "#b39595" },
    { name: t("statuses.pending"), value: pendingBookings, color: "#c4a882" },
    { name: t("statuses.cancelled"), value: cancelledBookings, color: "#c97a7a" },
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <KpiCard
          ...
          sub={
            <div className="flex items-center mt-1 gap-1">
              ...
              <span className={`text-xs font-medium ${bookingGrowth >= 0 ? "text-liberty" : "text-[#c97a7a]"}`}>
                {Math.abs(bookingGrowth)}% {t("analytics.vsPrev")}
              </span>
            </div>
          }
        ...
        <KpiCard
          ...
          sub={
            <p className="text-text-body text-xs mt-1">
              {totalBookings > 0 ? ((completedBookings / totalBookings) * 100).toFixed(0) : 0}% {t("analytics.rate")}
            </p>
          }
        ...
        <KpiCard
          ...
          sub={
            <p className="text-text-body text-xs mt-1">
              {totalBookings > 0 ? ((cancelledBookings / totalBookings) * 100).toFixed(0) : 0}% {t("analytics.rate")}
            </p>
          }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        <div className="lg:col-span-2 rounded-2xl p-6 bg-white border border-[#ede8e4]">
          <div className="flex items-center justify-between mb-5">
            <h3 className="text-base font-semibold text-liberty-dark">{t("analytics.generalStats")}</h3>
            <div className="flex gap-2">
              <Chip active>{t("analytics.bookings")}</Chip>
              <Chip>{t("analytics.revenue")}</Chip>
            </div>
          </div>
          <div className="h-[250px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={dailyData}>
                ...
                <Area type="monotone" dataKey="bookings" stroke="#b39595" strokeWidth={2.5} fillOpacity={1} fill="url(#colorBookings)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="rounded-2xl p-6 bg-white border border-[#ede8e4]">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-base font-semibold text-liberty-dark">{t("analytics.statusSplit")}</h3>
          ...
          <div className="flex items-center justify-center h-[190px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={statusPieData} cx="50%" cy="50%" innerRadius={58} outerRadius={78} paddingAngle={3} dataKey="value">
          ...
          <div className="mt-4 space-y-2.5">
            {statusPieData.map((item, idx) => (
              <div key={idx} className="flex items-center justify-between text-sm">
                ...
                <span className="font-semibold text-liberty-dark">{item.value}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="rounded-2xl p-6 bg-white border border-[#ede8e4]">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-base font-semibold text-liberty-dark">{t("analytics.services")}</h3>
            <span className="text-liberty text-xs font-medium cursor-pointer hover:text-liberty-dark transition-colors">
              {t("analytics.viewAllDetails")}
            </span>
          </div>
          <div className="h-[250px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={serviceData}>
                ...
                <XAxis dataKey="name" tick={{ fontSize: 10, fill: "#6a6a6a" }} tickLine={false} axisLine={false} interval="preserveStartEnd" angle={-15} textAnchor="end" height={40} />
                <YAxis hide />
                <Tooltip contentStyle={tooltipStyle} />
        ...
        <div className="rounded-2xl p-6 bg-white border border-[#ede8e4]">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-base font-semibold text-liberty-dark">{t("analytics.dailyPattern")}</h3>
            <span className="text-liberty text-xs font-medium cursor-pointer hover:text-liberty-dark transition-colors">
              {t("analytics.details")}
            </span>
          </div>
          <div className="h-[250px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={dailyData.slice(-10)}>
                <CartesianGrid strokeDasharray="3 3" stroke="#ede8e4" vertical={false} />
                <XAxis dataKey="date" tick={{ fontSize: 10, fill: "#6a6a6a" }} tickLine={false} axisLine={false} interval="preserveStartEnd" />
                <YAxis hide />
                <Tooltip contentStyle={tooltipStyle} />
        ...
        <div className="rounded-2xl p-6 bg-white border border-[#ede8e4]">
          <h3 className="text-base font-semibold mb-4 text-liberty-dark">{t("analytics.performance")}</h3>
          <div className="flex items-center justify-center mb-5">
            <div className="relative w-[140px] h-[140px]">
              <ResponsiveContainer width="100%" height="100%">
                <RadialBarChart cx="50%" cy="50%" innerRadius="80%" outerRadius="100%" data={radialData} startAngle={90} endAngle={-270}>
                  <RadialBar background={{ fill: "#ede8e4" }} dataKey="value" cornerRadius={10} fill="#5D6B8D" />
                </RadialBarChart>
              </ResponsiveContainer>
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                <p className="text-3xl font-bold text-liberty-dark">{completionRate}%</p>
                <p className="text-text-body text-xs">{t("analytics.completionRate")}</p>
              </div>
            </div>
          </div>
            ...
            {specialistData.slice(0, 4).map((specialist, idx) => {
              ...
              return (
                <div key={idx} className="flex items-center gap-2">
                  ...
                  <span className="text-sm flex-1 min-w-0 truncate text-text-body">{specialist.name}</span>
                  <div className="w-16 rounded-full h-1.5 flex-shrink-0 bg-[#ede8e4]">
                    <div className="h-1.5 rounded-full bg-liberty" style={{ width: `${pct}%` }} />
                  </div>
                  <span className="text-xs font-semibold flex-shrink-0 text-liberty-dark">{specialist.count}</span>
                </div>
              );
            })}
          </div>
...
interface KpiCardProps {
  ...
  sub: React.ReactNode; icon: React.ReactNode; chart: React.ReactNode;
}
...
interface ChipProps { children: React.ReactNode; active?: boolean; }

const Chip = ({ children, active }: ChipProps) => (
  <button className={`px-3 py-1 text-xs rounded-full font-medium transition-colors ${active ? "bg-primary/20 text-secondary" : "text-text-body hover:bg-primary/10"}`}>
    {children}
  </button>
);